...
module Maven
  ...
  class MetadataListener < XML::SAX::Document
    ...
    def start_element(element, _attributes)
      @stack.push(element)
    end

    def end_element(_element)
      @stack.pop
    end
  ...
  def mvn_cache(artifact)
    ...
    begin
      ...
      raise "Can't download artifact \"#{artifact}\" form #{@@repositroy}: #{e.message}"
    end
  end
  ...
  def mvn(group_id, artifact_id, version)
    ...
    begin
      ...
      raise "Can't download dependencies of \"#{artifact}\" : #{e.message}"
    end
  end
